...
export function cliHelpSpec(version: string): CliHelpSpec {
  return {
    ...
    commands: [
      ...
      ] },
      {
        ...
        subcommands: [
          ...
          { name: 'list', usage: 'product list [filters] [--json]', description: 'Query products (alias: query); defaults to page 1 (page size 1000).', flags: [
            ...
            { flag: '--include <fields>', description: 'Child collections to include (comma-separated). Empty string = basic product data only; omit/null = no product data. Valid: Names, ShortTexts, LongTexts, TechTexts, Items, Prices, Categories, Parameters, Variants, Markets, Images, Feeds, Urls, ShippingFees, RelatedProducts, DiscountCampaigns, LowestPrice.' },
          ], examples: ['geins product list --brand 1 --in-stock', 'geins product list --include Names,Prices,Images', 'geins product list --page 2 --batch <BatchId> --json'] },
      ...
      {
        name: 'campaign',
        description: 'Manage campaigns, incl. promocode (discount-code) campaigns. Uses the Management API (/API/Campaign/*).',
        api: 'Management API',
        subcommands: [
          { name: 'list', usage: 'campaign list [--json]', description: 'List campaigns (summary rows). GET /API/Campaign/List.', examples: ['geins campaign list --json'] },
          { name: 'types', usage: 'campaign types [--json]', description: 'List discount type ids — fetch before create to pick CampaignTypeId (e.g. 3=Percentage, 4=Fixed amount). GET /API/Campaign/Types.' },
          { name: 'get', usage: 'campaign get <id> [--json]', description: 'Get one campaign in full. GET /API/Campaign/{id}.' },
          {
            name: 'create',
            usage: "campaign create --promocode <CODE> --market <id> (--percentage <n> | --amount <CUR>:<n>) [--title <t> --lang <code>] [--from <iso>] [--to <iso>] [--usage-limit <n>] [--once-per-customer] [--priority <n>] [--enabled|--disabled]  |  [--file <path> | --body '<json>' | stdin]",
            description: 'Create a campaign. POST /API/Campaign. Promocode flags build a code-base campaign (CampaignBaseType=code); --percentage uses type 3, --amount uses type 4. The --file/--body/stdin form takes a full CampaignDetailItemBase body.',
            flags: [
              { flag: '--promocode <CODE>', description: 'Discount code (required for flag mode).' },
              { flag: '--market <id>', description: 'Market id (required; see `geins account markets`).' },
              { flag: '--percentage <n>', description: 'Percentage discount (CampaignTypeId 3).' },
              { flag: '--amount <CUR>:<n>', description: 'Fixed-amount discount per currency (CampaignTypeId 4); repeatable.' },
              { flag: '--title <text>', description: 'Localized title; pair with --lang.' },
              { flag: '--lang <code>', description: 'Language code for --title (default en).' },
              { flag: '--from <iso>', description: 'Valid from (default: now).' },
              { flag: '--to <iso>', description: 'Valid to.' },
              { flag: '--usage-limit <n>', description: 'Max total redemptions.' },
              { flag: '--once-per-customer', description: 'Limit to one redemption per customer.' },
              { flag: '--priority <n>', description: 'Campaign priority.' },
              { flag: '--enabled | --disabled', description: 'Enabled state (default: enabled).' },
            ],
            examples: [
      ...
      {
        name: 'merchant',
        description: 'Storefront data via the Merchant API (GraphQL) — products for sale in a sales channel, carts, and checkout token generation. Needs an api-key profile (apikey) plus a merchant context (merchant config set). Channel/market/locale resolve per field: flag → GEINS_* env → value stored on the active profile.',
        api: 'Merchant API (GraphQL)',
        subcommands: [
          { name: 'config', usage: 'merchant config [--json]', description: 'Show the resolved merchant context (account-name, channel, tld, market, locale, environment, channelId).' },
          { name: 'config set', usage: 'merchant config set [--channel <c>] [--tld <t>] [--market <m>] [--locale <l>] [--store-account <slug>] [--environment prod|qa|dev] [--success/--cancel/--continue/--terms/--privacy <url>] [--default-payment <id>] [--default-shipping <id>] [--customer-type person|organization] [--branding <json|@file>]', description: 'Persist storefront context AND default checkout settings (redirect URLs, branding, default payment/shipping, customer type) onto the active api-key profile — baked into every token, restored when switching accounts. NB: the storefront account slug uses --store-account, since --account-name is the reserved global v2-account flag.', examples: ['geins merchant config set --channel 1 --tld se --market se --locale sv-SE --store-account labs', 'geins merchant config set --success https://shop.com/thanks --terms https://shop.com/terms'] },
          { name: 'product search', usage: 'merchant product search [text] [--category <alias>] [--brand <alias>] [--take N] [--skip N] [--json]', description: 'Search products for sale in the channel. Returns name, price, and stock.', examples: ['geins merchant product search shoe --take 5 --json'] },
          { name: 'product', usage: 'merchant product <productId|term> [--json]', description: 'Full detail for one product (numeric = productId, else first search match).' },
          { name: 'categories', usage: 'merchant categories [--json]', description: 'List categories (id · name · alias) for filtering by categoryAlias. Alias: category.' },
          { name: 'brands', usage: 'merchant brands [--json]', description: 'List brands (id · name · alias) for filtering by brandAlias. Alias: brand.' },
          { name: 'cart create', usage: 'merchant cart create [--json]', description: 'Create a cart; prints its id (the API mints one via getCart with no id).' },
          { name: 'cart get', usage: 'merchant cart get <id> [--json]', description: 'Show a cart: items and total.' },
          { name: 'cart add', usage: 'merchant cart add <id> --sku <skuId> [--qty N] [--json]', description: 'Add an item to a cart (mutation addToCart).', examples: ['geins merchant cart add <cartId> --sku 123 --qty 2'] },
          { name: 'cart update', usage: 'merchant cart update <id> --item <itemId> --qty <n> [--json]', description: 'Change a cart item quantity (mutation updateCartItem).' },
          { name: 'cart remove', usage: 'merchant cart remove <id> --item <itemId> [--json]', description: 'Remove a cart item (updateCartItem with quantity 0).' },
          { name: 'cart promo', usage: 'merchant cart promo <id> <code> [--json]', description: 'Apply a promotion/discount code to a cart.' },
          { name: 'token', usage: 'merchant token <cartId> [--url] [--payment <id>] [--shipping <id>] [--available-payments <csv>] [--available-shipping <csv>] [--customer-type person|organization] [--editable] [--no-copy] [--success/--cancel/--continue/--terms/--privacy <url>] [--branding <json|@file>] [--user <json|@file>] [--json]', description: 'Generate a checkout token from a cart — a client-side unsigned JWT the hosted Geins checkout consumes (encoded identically to @geins/sdk). Per-token flags override the profile\'s persisted checkout defaults. --url prints the full https://checkout.geins.services/<token> link. The success URL gets Geins params appended (geins-cart/geins-pm/geins-pt/geins-uid) so the confirmation page receives the order. Requires full merchant context.', examples: ['geins merchant token <cartId> --url', 'geins merchant token <cartId> --success https://shop.com/thanks --json'] },
          { name: 'token parse', usage: 'merchant token parse <token>', description: 'Decode a checkout token payload (debug).' },
        ],
      },
